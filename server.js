...
mongoose.connect('mongodb://localhost:27017/recipes2',{useNewUrlParser:true,useUnifiedTopology:true});
...
server.put('/updateRecipe/:id',updateHandeler)
function recipesHandeler(req,res){
    console.log(req.query);
    const ingredient=req.query.ingredient;
    const url =`https://api.edamam.com/search?q=${ingredient}&app_id=${process.env.FOOD_API_ID}&app_key=${process.env.FOOD_API_KEY}`;
    axios.get(url).then(result =>{
        // console.log(result.data.hits);
        const recipeArray=result.data.hits.map(recipe =>{
...
function getFavoriteHandler (req,res){
 ...
 function deleteHandler (req,res){
     ...
     myRecipeModel.remove({_id:id},(error,data)=>{
